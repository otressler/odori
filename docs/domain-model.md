...
| `canonical_ingredient` | id, household_id, name, category_id, aliases, active | Household-scoped stable identity shared by recipes, inventory, and lists. |
...
| `recipe_source` | id, household_id, type, URL/file reference, content_hash, attribution, imported_at | Type is `manual`, `url`, `image`, `pdf`, or `generated`. |
| `import_job` | id, household_id, source_id, state, provider, attempts, error_code, raw_result_ref | State is `queued`, `processing`, `awaiting_review`, `completed`, `failed`. |
...
| `recommendation_run` | id, household_id, requested_at, input_snapshot, scoring_version, model_version | Stores reproducibility metadata, not an implicit recipe mutation. `model_version` is absent for deterministic-only runs. |
...
- Content-hash uniqueness for an import is scoped by household and processing schema/provider version; the same source may be reprocessed after a schema change without overwriting an approved recipe.
