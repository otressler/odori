...
| --- | --- | --- |
| `user` | id, display_name, locale, role | Single household initially; retain ownership for future sharing. |
...
| `inventory_item` | id, canonical_ingredient_id, status, updated_at | One active row per household and canonical ingredient. |
| `inventory_event` | id, item_id, type, previous_status, new_status, actor_id, origin | Append-only audit record. |
| `meal_plan` | id, week_start_date, owner_id | Week starts on the locale-configured first day. |
...
| `shopping_list` | id, name, plan_id, state, generated_at | State is `active`, `completed`, or `archived`. |
| `shopping_item` | id, list_id, canonical_ingredient_id, label, source, state, recipe_refs | `source`: calculated/manual; `state`: open/purchased/skipped. |
...
- Week plans are unique per owner and week start date.
