...
| FR-12 | Warn about removal of planned stock. | A manual status change away from `in_stock` warns when an upcoming meal requires the item; cooking that planned recipe does not produce this warning. |
...
5. **Household collaboration:** membership/roles, WebSocket-backed shared inventory and shopping, stale-write handling, and planned-stock conflict warnings.
