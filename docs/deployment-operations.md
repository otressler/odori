...
| `AZURE_OPENAI_RECIPE_IMPORT_DEPLOYMENT`, `AZURE_OPENAI_RECIPE_IMPORT_TIMEOUT_SECONDS`, `AZURE_OPENAI_RECIPE_IMPORT_MAX_OUTPUT_TOKENS` | Select the Microsoft Foundry deployment with web search enabled for URL recipe extraction, set its worker timeout (default `45` seconds), and set its response budget (default `4000` tokens). |
